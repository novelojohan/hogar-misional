...
export default function Pensiones2() {
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (error) {
      alert("Error: Es posible que alguien más haya ganado la fecha hace un momento.");
      console.error(error);
      setIsLoading(false);
      return;
    }
  ...
  return (
    <main className="relative min-h-screen flex flex-col items-center p-5 md:p-8 pt-24 z-0">
      ...
      <div className="w-full max-w-md space-y-6 mt-4">
        ...
        >
          <h1 className="text-3xl font-extrabold text-iglesia-blue tracking-tight mb-1">Pensiones 2</h1>
          <h2 className="text-[#d4af37] font-bold text-sm mb-6">Hna. Galván y Hna. Aquino</h2>
          
          <div className="flex flex-col items-center gap-3">
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.40em]">Restricciones</span>
            <div className="space-y-3">
              <div className="flex flex-col items-center gap-0.5">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Hna. Galván</span>
                <span className="text-xs font-medium text-slate-500 italic max-w-[260px] leading-relaxed text-center">
                  Leche, carne asada de res, salchicha, jamón, longaniza y pepperoni.
                </span>
              </div>
              <div className="flex flex-col items-center gap-0.5">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Hna. Aquino</span>
                <span className="text-xs font-medium text-slate-500 italic">Cacahuate.</span>
              </div>
            </div>
          </div>
        </motion.section>
        ...
        >
          ...
          <div className="flex gap-8 mt-6 pt-6 border-t border-slate-200/60 w-full justify-center">
            <div className="flex items-center gap-2 text-[10px] font-bold text-emerald-600 uppercase tracking-widest">
              <div className="w-3 h-3 rounded-full bg-emerald-600"></div> Reservado
            </div>
      ...
      <AnimatePresence mode="wait">
        {isModalOpen && selectedDate && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-slate-900/40 backdrop-blur-md flex justify-center items-center z-[100] p-4">
            <motion.div initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 30, opacity: 0 }} transition={{ duration: 0.8, ease: "easeOut" }} className="bg-white/95 backdrop-blur-xl w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden relative border border-white">
              <button onClick={() => { setIsModalOpen(false); setSelectedDate(undefined); }} className="absolute top-6 right-6 text-slate-400 hover:text-slate-800 bg-slate-100 rounded-full p-2 transition-colors">
                <X size={20} />
              </button>
                ...
                {existingBooking ? (
                  ...
                  <form onSubmit={handleFormSubmit} className="space-y-5">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input required name="responsible" type="text" placeholder="¿Quién recibe?" className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-4 text-sm focus:ring-2 focus:ring-[#d4af37] outline-none transition-all" />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.15em] ml-2">WhatsApp</label>
                        <input required name="phone" type="tel" pattern="[0-9]{10}" maxLength={10} minLength={10} placeholder="10 dígitos" className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-4 text-sm focus:ring-2 focus:ring-[#d4af37] outline-none transition-all" />
                      </div>
                      <div className="space-y-1.5">
